...
#dataset folder
conf$root = '../danieleFolder/data/cattle_SNPs'
...
runName = 'Dfract0_EpiAddOv-25'
conf$pheno_outfile = file.path(conf$root, runName, 'simulated_phenotype.csv')
#SNP matrix without the causative effect (we are always saving a .gz file)
conf$SNP_outfile = file.path(conf$root, runName, 'SNPs_nonCausative.csv.gz') 
#text file with list of causative SNPs
conf$SNPlist_outfile = file.path(conf$root, runName, 'SNPs_causative.list.txt') 
#parameter used for the simulation, just for checks
conf$params_outfile = file.path(conf$root, runName, 'usedpars.txt') 
